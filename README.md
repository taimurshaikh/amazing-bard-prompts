...
- Please review whether the prompt works correctly in Google Bard using the Excel sheet for the next Bard. After reviewing or modifying the prompt, producing the prompt, and modifying the CSV file, please add the contributor's GitHub address next to the CSV file and submit a pull request. The Markdown file will be automatically generated based on the CSV file. Please only modify the contents in the [bard_prompts]("./bard_prompts") folder.
- 다음 바드용 엑셀에서 바드에서 해당 프롬프트가 정상적으로 작동하는지 검수하세요. 검수 혹은 프롬프트를 수정, 프롬프트를 생산하신다음에 csv파일을 수정하고, csv파일 옆에 기여자에 깃허브 주소를 적어주신다음에 pull_request를 해주세요. csv파일을 바탕으로 markdown파일은 자동으로 생성됩니다. 반드시 [bard_prompts]("./bard_prompts") 폴더 안에 있는 내용만 수정해주세요. 
- 次のBard用のExcelシートを使用して、プロンプトがGoogle Bardで正常に機能しているかどうかを確認してください。プロンプトを見直したり、修正したり、生成したりした後、CSVファイルを修正し、CSVファイルの横に貢献者のGitHubアドレスを追加してプルリクエストを送信してください。MarkdownファイルはCSVファイルを基に自動的に生成されます。 [bard_prompts]("./bard_prompts") フォルダー内の内容のみを変更してください。
